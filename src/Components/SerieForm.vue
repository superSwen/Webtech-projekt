...
import type { SerieDto, SerieCreateUpdate } from '@/types/media'
...
const form = reactive<SerieCreateUpdate>({
  title: '',
  season: null,
  episode: null,
  minutes: null,
...
watch(
  ...
  (val) => {
    if (val) {
      ...
      form.season = val.season ?? null
      form.episode = val.episode ?? null
...
<template>
  <section class="card">
    <h2>{{ isEdit ? 'Serie bearbeiten' : 'Neue Serie anlegen' }}</h2>
    ...
    <form @submit.prevent="onSubmit" class="form">
      ...
      <div class="grid">
        <label>
          Staffel
          <input v-model.number="form.season" :disabled="busy" type="number" min="1" />
        </label>

        <label>
          Episode
          <input v-model.number="form.episode" :disabled="busy" type="number" min="1" />
        </label>
      </div>
      ...
      <div class="actions">
        <button class="btn primary" type="submit" :disabled="busy">
          {{ isEdit ? 'Änderungen speichern' : 'Serie speichern' }}
        </button>
