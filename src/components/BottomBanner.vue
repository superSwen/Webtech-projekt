...
const props = withDefaults(
  defineProps<{ refreshKey: number; position?: 'top' | 'bottom' }>(),
  { position: 'bottom' }
)
...
<template>
  <div :class="props.position === 'bottom' ? 'mt-8' : ''">
    <div
      v-if="loading && !banner"
    ...
    <div v-else-if="banner" class="relative overflow-hidden rounded-2xl border border-white/10">
      <div class="absolute inset-0 bg-cover bg-center" :style="{ backgroundImage: `url(${banner.imageUrl})` }" />
      ...
      <div class="relative flex h-[clamp(180px,22vw,280px)] flex-col justify-center gap-2 p-5">
        <div class="text-xs text-white/70">
          Zufällig aus deinen Serien <span class="opacity-60">•</span> Serie
        </div>
