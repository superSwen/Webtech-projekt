...
import tanjiroUrl from '@/assets/tanjiro.png'
import ankoUrl from '@/assets/anko.png'

const props = withDefaults(
  defineProps<{ enabled?: boolean; size?: 'card' | 'header' }>(),
  { enabled: true, size: 'card' }
)
...
const wrapperClass = computed(() => {
  return props.size === 'header' ? 'relative h-14 w-28' : 'relative h-44 w-72'
})
...
<template>
  <div v-if="hasHero" :class="wrapperClass">
...
@keyframes shine-sweep {
  0% { transform: translateX(-170%) rotate(14deg); opacity: 0; }
  12% { opacity: 0.75; }
  55% { opacity: 0.35; }
  100% { transform: translateX(170%) rotate(14deg); opacity: 0; }
}
.heroShine { animation: shine-sweep 3.2s ease-in-out infinite; }
...
.heroFloat { animation: floaty 6s ease-in-out infinite; }

@media (prefers-reduced-motion: reduce) {
  .heroShine, .heroFloat { animation: none !important; }
}
</style>
